...
// Baseado na estrutura do prompt-template.js
...
async function processLine(line, anthropic, lineNumber) {
    ...
    while (currentTry <= config.maxRetries) {
        try {
            ...
            const prompt = `Você é um especialista em processamento de linguagem natural e marcação de texto. Sua tarefa é adicionar tags XML especiais ao texto abaixo, que contém informações sobre palavras em Yanomami.

O texto segue um formato específico que compara duas palavras em Yanomami, incluindo seus significados, categorias gramaticais, usos e exemplos.

Adicione as seguintes tags XML ao texto:
- <WORD>palavra_yanomami</WORD> - para marcar palavras em Yanomami
- <POS>categoria_gramatical</POS> - para categorias gramaticais (Noun, Verb, Adjective, etc.)
- <DEFINITION>definição</DEFINITION> - para definições de palavras
- <EXAMPLES>exemplos</EXAMPLES> - para a seção de exemplos
- <EXAMPLE_YANOMAMI>exemplo_em_yanomami</EXAMPLE_YANOMAMI> - para exemplos em Yanomami
- <EXAMPLE_TRANSLATION>tradução_do_exemplo</EXAMPLE_TRANSLATION> - para traduções de exemplos
- <YANOMAMI>texto_em_yanomami</YANOMAMI> - para outras frases em Yanomami
- <TRANSLATION>tradução</TRANSLATION> - para traduções
- <LITERAL>tradução_literal</LITERAL> - para traduções literais
- <RELATED_FORMS>formas_relacionadas</RELATED_FORMS> - para formas relacionadas
- <USAGE>uso</USAGE> - para informações sobre uso
- <GRAMMATICAL>informação_gramatical</GRAMMATICAL> - para informações gramaticais adicionais

Regras:
1. Preserve o formato e a estrutura original do texto
2. Não adicione ou remova informações
3. Aplique as tags de forma consistente e precisa
4. Certifique-se de que todas as tags estejam corretamente fechadas
5. Não use tags aninhadas do mesmo tipo

Aqui está o texto para marcar:

${line}

Responda APENAS com o texto marcado, sem explicações adicionais.`;
